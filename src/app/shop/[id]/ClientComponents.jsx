...
export function ProductHero({ pc }) {
  ...
  useGSAP(() => {
    ...
    // Scroll parallax animation for the monolith
...
export function ProductSpecs({ pc }) {
  ...
  const specsDetails = [
    { label: "Processor", value: pc.specs.cpu, icon: Cpu, desc: "Unlocked architecture for maximum multi-core capability." },
    { label: "Graphics", value: pc.specs.gpu, icon: Zap, desc: "Ray tracing and AI-accelerated performance for flawless rendering." },
    { label: "Memory", value: pc.specs.ram, icon: MemoryStick, desc: "Ultra-low latency memory channels for unbottlenecked speed." },
...
export function StickyPurchaseBar({ pc }) {
  const barRef = useRef(null);

  useGSAP(() => {
    // Only show bar after scrolling past the hero
  ...
  return (
    <div ref={barRef} className="sticky-purchase-bar" style={{ position: 'fixed', bottom: 32, left: '50%', transform: 'translateX(-50%)', zIndex: 100, background: 'rgba(10,10,10,0.8)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 100, padding: '12px 12px 12px 32px', display: 'flex', alignItems: 'center', gap: 48, boxShadow: '0 20px 40px -10px rgba(0,0,0,0.5)' }}>
      ...
      <button className="purchase-cta" style={{ background: 'var(--cs-cream)', color: '#111', border: 'none', padding: '16px 32px', borderRadius: 40, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', transition: 'transform 200ms ease' }}>
        Add to build <ArrowRight size={16} />
      </button>
    </div>
  );
}
